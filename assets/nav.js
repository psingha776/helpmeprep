...
// `title` is a short topic label (3-4 words max). `file` is the basename without .html.
const NAV_DAYS = [
...
function buildNav() {
  ...
  const html = NAV_DAYS.map((d) => {
    ...
    const subItems = d.isMock
      ? [
          { label: "Mock Test 1", href: `${d.file}.html#mock1` },
          { label: "Mock Test 2", href: `${d.file}.html#mock2` },
        ]
      : [
          { label: "Study", href: `${d.file}.html#study` },
          { label: "Quiz",  href: `${d.file}.html#quiz`  },
        ];
    const subHtml = subItems
      .map(
        (s) =>
          `<li><a href="${s.href}" class="${isCurrent ? "current" : ""}">${s.label}</a></li>`
      )
      .join("");
    return `
      <div class="nav-day ${isCurrent ? "open" : ""}">
        <button class="nav-day-header ${isCurrent ? "active" : ""}" onclick="toggleNavDay(this)" aria-expanded="${isCurrent}">
          <span class="nav-day-title">
            <span class="day-num">${d.num}</span>
            <span>Day ${d.num} · ${d.title}</span>
          </span>
          <span class="chevron">›</span>
        </button>
        <ul class="nav-sublist">${subHtml}</ul>
      </div>
    `;
  }).join("");
...
function toggleNavDay(btn) {
  ...
  const expanded = day.classList.contains("open");
