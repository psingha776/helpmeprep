...
function reRender(sel,html){const el=document.querySelector(sel);if(!el)return;const tmp=document.createElement('div');tmp.innerHTML=html;el.replaceWith(tmp.firstElementChild);}
...
function updateNotesBadge(dayNum){
  ...
  if(b){b.textContent=n?`📝 ${n} note${n>1?'s':''}`:'0 notes — click "+ note" beside any heading';b.style.display='inline-block';b.classList.toggle('empty',!n);}
}
...
function maybeShowStickyCoachmark(){
  ...
  cm.innerHTML=`<div class="cm-arrow">↑</div><div class="cm-body"><strong>Tip:</strong> Click <em>+ note</em> beside any heading to leave yourself a sticky note here. Notes show up on your home page too.</div><button class="cm-x" title="Got it" onclick="dismissCoachmark(this)">Got it</button>`;
...
function maybeShowKbdCoachmark(){
  ...
  t.innerHTML=`<div><strong>⌨ Keyboard shortcuts:</strong> <kbd>1</kbd>–<kbd>4</kbd> answer · <kbd>H</kbd> hint · <kbd>→</kbd> next · <kbd>?</kbd> help</div><button onclick="dismissKbdToast(this)" title="Dismiss">✕</button>`;
...
function updateXPDisplay(){const el=document.getElementById('xpDisplay');if(el){const l=getLevel();el.innerHTML=`⚡ ${getXP()} XP · Lv${l+1} ${XP_L[l]}<br>🔥 ${getStreak()}-day streak`;}}
function showXPToast(amt,anchor){const t=document.createElement('div');t.className='xp-toast';t.textContent=`+${amt} XP`;const r=anchor?anchor.getBoundingClientRect():{left:window.innerWidth/2,top:200};t.style.cssText=`left:${r.left+20}px;top:${r.top+window.scrollY-10}px`;document.body.appendChild(t);setTimeout(()=>t.remove(),1000);}
function flashLevelUp(){const el=document.getElementById('xpDisplay');if(el){el.classList.add('level-up-flash');setTimeout(()=>el.classList.remove('level-up-flash'),2000);}}
...
function renderHearts(){const hr=document.getElementById('heartsRow');if(hr)hr.innerHTML=Array.from({length:5},(_,i)=>`<span class="${i<_lives?'':'heart-lost'}">${i<_lives?'❤️':'🖤'}</span>`).join('');}
...
function showLockOverlay(mins){const ov=document.createElement('div');ov.className='lock-overlay';ov.innerHTML=`<div style="font-size:3rem">🔒</div><h2 style="color:#fff">Session Locked</h2><p style="color:#ccc">All hearts lost. Come back in ~${mins} min.</p>`;document.body.appendChild(ov);}
...
function revealHint(qi){
  ...
  if(used===0){const wrongs=qd.co.map((_,di)=>di).filter(di=>di!==qd.nc);const grey=wrongs[Math.floor(Math.random()*wrongs.length)];const btn=block.querySelectorAll('.choice')[grey];if(btn){btn.classList.add('hint-greyed');btn.disabled=true;}hb.innerHTML+=`<div class="hint-label">Hint 1</div><span>One unlikely answer dimmed.</span>`;}
  else if(used===1){hb.innerHTML+=`<div class="hint-label">Hint 2</div><span>${esc(sents[0].trim())}</span>`;}
  else {const domain=q.domain?`[${q.domain}] `:'';const last=sents[sents.length-1].trim();hb.innerHTML+=`<div class="hint-label">Hint 3</div><span>${esc(domain+last)}</span>`;const btn=block.querySelector('.hint-btn');if(btn){btn.disabled=true;btn.textContent='All hints used';}}
}
...
function renderAll(){
  ...
  const reviewHTML=_reviewCards.length?`<div class="review-section-header">📋 Spaced Review — Questions from Earlier Days <span class="review-count">(${_reviewCards.length} queued)</span></div>`+_reviewCards.map((rc,ri)=>renderReviewCard(ri,rc)).join(''):'';
...
function renderReviewCard(ri,rc){
  ...
  const expl=answered?`<div class="explanation show ${ok?'correct':'wrong'}"><div class="explanation-title">${ok?'✓ Correct':'✗ Not quite'}</div><details class="show-explanation"><summary>Show explanation</summary><div class="explanation-text">${esc(item.explanation||'')}</div></details>${buildAskGoogle(item.domain||'',item.text)}</div>`:'<div class="explanation"></div>';
...
function pickReview(ri,di){const rc=_reviewCards[ri];if(rc.answered)return;rc.answered=true;rc.sel=di;rc.ok=di===rc.nc;if(rc.ok){removeFromPool(rc.item.id);addXP(15);showXPToast(15,document.querySelector(`[data-r="${ri}"]`));}reRender(`[data-r="${ri}"]`,renderReviewCard(ri,rc));updateProg();checkDone();}
...
function renderMCQ(qi,qd){
  ...
  const expl=qd.answered?`<div class="explanation show ${qd.ok?'correct':'wrong'}"><div class="explanation-title">${qd.ok?'✓ Correct':'✗ Not quite'} ${timerHTML}</div><details class="show-explanation"><summary>Show explanation</summary><div class="explanation-text">${esc(q.explanation||'')}</div></details>${buildAskGoogle(q.domain||'',q.text)}</div>`:'<div class="explanation"></div>';
...
function pickMCQ(qi,di){
  ...
  if(qd.ok){const xp=xpTable[Math.min(hu,3)]+(qd.conf==='certain'?2:0);addXP(xp);persist();reRender(`[data-q="${qi}"]`,renderMCQ(qi,qd));showXPToast(xp,document.querySelector(`[data-q="${qi}"]`));}
  else {addToPool(`${storageKey()}_${qi}`,q,'wrong');loseHeart(dayNum);persist();reRender(`[data-q="${qi}"]`,renderMCQ(qi,qd));}
...
function renderMatch(qi,qd){
  const q=QUESTIONS[qd.i];
  if(qd.answered){const rows=q.pairs.map(p=>`<div class="match-row"><span class="match-term correct-pair">${esc(p.term)}</span><span class="match-arrow">→</span><span class="match-def correct-pair">${esc(p.definition)}</span></div>`).join('');return `<div class="question match-q" data-q="${qi}"><div class="question-num">Q${qi+1} of ${QUESTIONS.length}</div><div class="question-text">${esc(q.text)}</div><div class="match-pairs">${rows}</div><div class="explanation show correct"><div class="explanation-title">✓ All matched</div><details class="show-explanation"><summary>Show explanation</summary><div class="explanation-text">${esc(q.explanation||'')}</div></details></div></div>`;}
...
function renderOrder(qi,qd){
  const q=QUESTIONS[qd.i];
  if(qd.answered){const items=q.correctOrder.map(ci=>`<div class="order-item correct"><span class="drag-handle">✓</span>${esc(q.items[ci])}</div>`).join('');return `<div class="question" data-q="${qi}"><div class="question-num">Q${qi+1} of ${QUESTIONS.length}</div><div class="question-text">${esc(q.text)}</div><div class="order-list">${items}</div><div class="explanation show correct"><div class="explanation-title">✓ Correct order</div><details class="show-explanation"><summary>Show explanation</summary><div class="explanation-text">${esc(q.explanation||'')}</div></details></div></div>`;}
...
function checkOrder(qi){
  ...
  if(ok){qd.answered=true;qd.ok=true;persist();const xp=qd.attempts===0?10:5;addXP(xp);reRender(`[data-q="${qi}"]`,renderOrder(qi,qd));showXPToast(xp,document.querySelector(`[data-q="${qi}"]`));updateProg();checkDone();}
  else {qd.attempts++;persist();if(qd.attempts>=2){qd.answered=true;qd.ok=false;qd.cur=[...q.correctOrder];persist();reRender(`[data-q="${qi}"]`,renderOrder(qi,qd));updateProg();checkDone();}else {document.querySelector(`[data-q="${qi}"]`).querySelectorAll('.order-item').forEach((item,pos)=>{item.classList.toggle('correct',qd.cur[pos]===q.correctOrder[pos]);item.classList.toggle('wrong',qd.cur[pos]!==q.correctOrder[pos]);});}}
}
...
function submitBlank(qi){const qd=quizData[qi];if(qd.answered||qd.selIdx===undefined)return;const q=QUESTIONS[qd.i];qd.answered=true;qd.ok=qd.selIdx===q.correct;if(qd.ok)addXP(10);persist();reRender(`[data-q="${qi}"]`,renderFillblank(qi,qd));if(qd.ok)showXPToast(10,document.querySelector(`[data-q="${qi}"]`));updateProg();checkDone();}
...
function resetQuiz(){try{localStorage.removeItem(storageKey());}catch{}Object.keys(_hintUsed).forEach(k=>delete _hintUsed[k]);quizData=buildFresh();persist();renderAll();const s=document.getElementById('quiz');if(s)window.scrollTo({top:s.offsetTop-20,behavior:'smooth'});}
function clearReviewQueue(){if(confirm('Clear all review cards from earlier days?')){localStorage.removeItem('hmp_review_pool');renderAll();}}
